...
| 校曆同步 + 活動連結 | 有 | — | **連結器核心**：附著校園活動 |
| 素材記錄（基本） | 有 | — | 把經歷變成素材 |
| 成就時光軸 | 有 | — | **成就回顧**：看到自己累積了什麼 |
...
#### Phase 1：MVP — 分數分析 + 圖卡（Week 1-2）
3 個功能，目標：學生能輸入分數、看結果、分享圖卡
  1. 分數分析器（學測版）
  2. 分享圖卡生成（繪馬式設計 + 「我的目標」+ 預設匿名 + 正向框架）
  3. Landing page + 學測倒數

#### Phase 2：連結器 — 路線圖 + 校曆 + 素材（Week 3-6）
4 個功能，目標：學生看到個人化路線圖、開始記錄校園活動
  4. **個人化升學路線圖（核心）**（見附錄 B：以終為始）
     - 輸入：年級 + 目標管道 + 目標科系 → 輸出：倒計時路線圖
     - 四階段規劃：素材收集 → 撰寫 → 備審準備 → 口試練習
     - 缺口分析 + 校曆活動匹配 + 學校資源地圖
  5. 校曆同步（手動輸入學校行事曆，系統自動連結學習歷程代碼）
  6. 素材記錄（基本版，按代碼分類，輕量記錄介面）
  7. 科系探索（58 學類基本資料）

#### Phase 3：回顧 + 家長視角（Week 7-9）
3 個功能，目標：學生看到自己的累積、家長開始看到進度
  8. 成就時光軸（不是 streak 壓力，是回顧「你這個月做了什麼」）
  9. 家長註冊 + 免費週報（摘要版）
  10. 升學管道資料頁（15+ 管道）

#### Phase 4：付費產品（Week 10-13）
3 個功能，目標：首筆家長付費
  11. 付費牆 + 訂閱系統
  12. 家長完整成長報告（詳細版 + AI 成長亮點摘要）
  13. AI 學習歷程審查

#### Phase 5：擴展（Week 14-22）
...
| Phase | 功能數 | 估計時間 | 累計 | 里程碑 |
|-------|:------:|:--------:|:----:|--------|
| **Phase 1 MVP** | **3** | **2 週** | 2 週 | 分數分析器 + 圖卡上線 |
| **Phase 2 連結器** | **4** | **4 週** | **6 週** | 個人化路線圖 + 校曆同步 |
| **Phase 3 回顧+家長** | **3** | **3 週** | **9 週** | 家長週報上線 |
| **Phase 4 付費** | **3** | **4 週** | **13 週（3 月）** | 首筆付費 |
| Phase 5 擴展 | 6 | 9 週 | 22 週（5.5 月） | 全功能上線 |
| Phase 6 規模化 | 4 | 4 週 | 26 週（6.5 月） | 規模化運營 |

**假設：一人開發，每天 4-6 小時。有設計師或第二開發者可砍半。**
**科系資料（58 學類）和管道資料（15+ 管道）是內容工作，可與開發並行。**
...
### Phase 2：連結器 — 校曆 + 素材（Week 3-5）

目標：學生開始記錄校園活動、探索科系

- [ ] Week 3：校曆同步
  - [ ] 學生手動輸入學校行事曆（事件名稱 + 日期 + 類型）
  - [ ] 系統自動將校園活動連結到學習歷程代碼
...
### Phase 3：成就回顧 + 家長視角（Week 6-8）

目標：學生看到自己的累積、家長開始看到進度

- [ ] Week 6：成就時光軸（見下方「成就系統設計」）
  - [ ] 時間軸視覺化（每月回顧：你這個月做了什麼）
  - [ ] 成長摘要（AI 自動生成正向描述）
  - [ ] 無 streak、無扣分、無壓力。純回顧、純鼓勵
- [ ] Week 6-7：家長註冊 + 免費週報
  - [ ] 家長用 email 註冊，綁定孩子帳號
  - [ ] 每週日自動推送（孩子的探索 + 素材新增 + 正向亮點）
  - [ ] 免費版只顯示摘要，不顯示細節
- [ ] Week 7-8：升學管道資料頁
  - [ ] 15+ 管道完整資料（手動整理）
  - [ ] 學測/統測/申請/分發等主要管道
...
### Phase 4：付費產品（Week 9-12）

目標：首筆家長付費

- [ ] Week 9：Supabase Auth + 用戶系統
- [ ] Week 9：付費牆（Stripe / LINE Pay）
- [ ] Week 9-10：家長完整成長報告
  - [ ] 每週詳細版（探索歷程、興趣變化、主動記錄）
  - [ ] AI 成長亮點（自動摘要孩子的正面表現）
  - [ ] 親子共同探索建議（基於興趣的 Open Day、科系推薦）
- [ ] Week 10-11：AI 學習歷程審查
  - [ ] 上傳學習歷程 → Gemini 分析 → 修改建議
  - [ ] 按科系分類的建議
  - [ ] 素材空缺分析
- [ ] Week 11-12：付費流程優化 + 用戶回饋

交付物：付費系統上線、首筆收入

### Phase 5：擴展（Week 13-20）

目標：每 2 週加 1 個功能，基於用戶回饋決定順序

- [ ] Week 13-14：面試模擬（AI 文字版 5 輪）
- [ ] Week 15-16：科系策略報告
- [ ] Week 16-17：MBTI 式科系測驗（病毒傳播第二引擎）
- [ ] Week 17-18：高職版（統測分數分析）
- [ ] Week 18-19：匿名結果牆
...
**這是連結器的核心功能。** 不是追蹤器，是規劃器。告訴學生「從現在到截止日，你需要完成什麼、什麼時候完成、哪些學校資源可以用」。
...
|------|------|------|
| 年級 | 高一/高二/高三上 | 手動選擇 |
| 目標管道 | 申請入學/繁星推薦/分發入學/四技甄選 | 手動選擇 |
| 目標科系學群 | 58 學類中選 1-3 個感興趣的 | 從 Phase 2 科系探索的數據 |
| 已有素材 | 手動輸入已有幾件、哪些代碼 | 從素材記錄的數據 |
...
**路線圖告訴學生「你還需要做什麼」，時光軸告訴學生「你已經做了什麼」。兩者互補。**
